 ...
 * @author Marty Stepp
 * @version 2021/04/17
 * - added get/setColorHelper, setStyleSheet
 * @version 2021/04/09
 * - added sgl namespace
 * @version 2019/04/23
...
std::string GInteractor::getBackground() const {
    return getColorHelper(getWidget()->backgroundRole());
}

int GInteractor::getBackgroundInt() const {
    return getColorHelperInt(getWidget()->backgroundRole());
}
...
std::string GInteractor::getColor() const {
    return getColorHelper(getWidget()->foregroundRole());
}

std::string GInteractor::getColorHelper(QPalette::ColorRole role) const {
    int rgb = getColorHelperInt(role);
    return GColor::convertRGBToColor(rgb);
}

int GInteractor::getColorHelperInt(QPalette::ColorRole role) const {
    QColor color = getWidget()->palette().color(role);
    return GColor::convertQColorToRGB(color);
}

int GInteractor::getColorInt() const {
    return getColorHelperInt(getWidget()->foregroundRole());
}
...
void GInteractor::setBackground(int rgb) {
    // TODO: refactor to use setColorHelperInt?
    GThread::runOnQtGuiThread([this, rgb]() {
        ...
        // TODO: move this specialization code into the widget subclasses?
        if (getType() == "GChooser") {
...
void GInteractor::setBackground(const std::string& color) {
    // TODO: refactor to use setColorHelper?
...
void GInteractor::setColorHelper(QPalette::ColorRole role, int rgb) {
    setColorHelper(role, QColor(rgb));
}

void GInteractor::setColorHelper(QPalette::ColorRole role, const QColor& qcolor) {
    GThread::runOnQtGuiThread([this, &role, &qcolor]() {
        QPalette palette(getWidget()->palette());
...
void GInteractor::setColorHelper(QPalette::ColorRole role, const std::string& color) {
    if (GColor::hasAlpha(color)) {
        int argb = GColor::convertColorToARGB(color);
        QColor qcolor = GColor::toQColorARGB(argb);
        setColorHelper(role, qcolor);
    } else {
        int rgb = GColor::convertColorToRGB(color);
        setColorHelper(role, rgb);
    }
}
...
void GInteractor::setForeground(int rgb) {
    setColorHelper(getWidget()->foregroundRole(), rgb);
}

void GInteractor::setForeground(const std::string& color) {
    setColorHelper(getWidget()->foregroundRole(), color);
}
...
void GInteractor::setStyleSheet(const std::string& styleSheet, bool replace) {
    GThread::runOnQtGuiThread([this, styleSheet, replace]() {
        if (replace) {
            getWidget()->setStyleSheet(QString::fromStdString(styleSheet));
        } else {
            QString style = getWidget()->styleSheet();
            style.append(QString::fromStdString(styleSheet));
