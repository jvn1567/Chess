...
const std::string GText::DEFAULT_FONT_FAMILY = "";   // query the OS for this
const int GText::DEFAULT_FONT_SIZE = 13;
...
std::string GText::getDefaultFont() {
    std::string family;
    if (DEFAULT_FONT_FAMILY.empty()) {
        QFont qfont;
        family = qfont.defaultFamily().toStdString();
    } else {
        family = DEFAULT_FONT_FAMILY;
    }
    return family + "-" + std::to_string(DEFAULT_FONT_SIZE);
}
